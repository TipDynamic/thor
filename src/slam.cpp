...
void slam::getKeyPointsColor(const cv::Mat &img1, const cv::Mat &img2, vector <KeyPoint> &kp1,
                             vector <KeyPoint> &kp2) {
  // calculate keypoints
  /**
...
void slam::getGoodMatchesColor(const cv::Mat &img1, const cv::Mat &img2, vector<cv::DMatch> &good_matches, vector <KeyPoint> &kp1,
                               vector <KeyPoint> &kp2, bool show) {
  // get good matches from images
...
//  Ptr<BFMatcher> matcher = BFMatcher::create(cv::NORM_HAMMING);
  Ptr<cv::FlannBasedMatcher> matcher;


  vector<DMatch> all_matches;
  cv::Mat descriptor1, descriptor2;
  detector->detectAndCompute(img1, Mat(), kp1, descriptor1);
  detector->detectAndCompute(img2, Mat(), kp2, descriptor2);
  ...
  // filter good matches
  // try different strategy here, less than 0.6 keep or more than 4* out
